...
/** Consistent "Ask McGyver" search button - pill shape, light border, used on every page */
export function SearchButton({ className = '' }: { className?: string }) {
  return (
    <Link
      ...
      aria-label="Ask McGyver - Search"
      title="Ask McGyver - Search"
    >
      <Search className="h-4 w-4 sm:h-[18px] sm:w-[18px] shrink-0" />
      <span className="text-sm truncate">Ask McGyver</span>
    </Link>
  );
}
