...
export function FollowButton({ targetUserId, currentUserId, targetIsPrivate, className = '', showWhenFollowing, onFollowChange }: FollowButtonProps) {
  ...
  return (
    <button
      ...
      onClick={handleClick}
      disabled={loading}
      className={`inline-flex items-center px-4 py-2 rounded-xl font-medium transition disabled:opacity-60 disabled:cursor-not-allowed ${
        following && !isPending
    ...
    >
      {loading ? 'Updating...' : isPending ? 'Request Sent' : following ? 'Following' : 'Follow'}
    </button>
  );
}
