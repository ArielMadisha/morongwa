...
import { ArrowLeft, Truck, Package, ArrowRight, Shirt } from 'lucide-react';
    ...
    <div className="min-h-screen bg-gradient-to-br from-sky-50 via-white to-sky-100 text-slate-800">
      ...
      <main className="mx-auto max-w-6xl px-6 py-8">
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          ...
          <Link
            href="/admin/dropship/shein"
            className="group flex items-start gap-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-lg transition hover:-translate-y-1 hover:shadow-xl hover:border-rose-200"
          >
            <div className="flex h-14 w-14 flex-shrink-0 items-center justify-center rounded-xl bg-rose-100 text-rose-600">
              <Shirt className="h-8 w-8" />
            </div>
            <div className="flex-1 min-w-0">
              <h2 className="text-xl font-semibold text-slate-900">SHEIN Dropshipping</h2>
              <p className="mt-1 text-sm text-slate-600">
                Search SHEIN Open Platform, import by SPU ID, sync stock. Pass-through catalog pricing (no Qwertymates markup).
              </p>
              <span className="mt-3 inline-flex items-center gap-1 text-sm font-medium text-rose-600 group-hover:text-rose-700">
                Open <ArrowRight className="h-4 w-4" />
              </span>
            </div>
          </Link>
