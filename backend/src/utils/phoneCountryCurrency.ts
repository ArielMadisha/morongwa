...
/** European ISO codes (geographic Europe + closely linked microstates) → display USD */
...
/** When libphonenumber cannot resolve, infer country from common E.164 prefixes */
...
/** ISO 3166-1 alpha-2 → ISO 4217 for app display */
...
export function detectCurrencyFromPhoneDigits(phoneDigits: string): string {
  ...
  if (d.startsWith("264")) return "NAD";
  if (d.startsWith("266")) return "LSL";
  if (d.startsWith("260")) return "ZMW";
  if (d.startsWith("263")) return "ZWL";
  if (d.startsWith("254")) return "KES";
  if (d.startsWith("255")) return "TZS";
  if (d.startsWith("256")) return "UGX";
  if (d.startsWith("250")) return "RWF";
  if (d.startsWith("251")) return "ETB";
  if (d.startsWith("234")) return "NGN";
  if (d.startsWith("233")) return "GHS";
  return "USD";
}

export function detectCountryCodeFromPhoneDigits(phoneDigits: string): string {
  return detectCountryIsoFromPhoneDigits(phoneDigits) || fallbackCountryIsoFromPrefix(phoneDigits) || "ZA";
}
