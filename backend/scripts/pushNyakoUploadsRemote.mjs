/**
 * SFTP nyako-* profile images to production.
 *   node scripts/pushNyakoUploadsRemote.mjs
 */
import fs from "fs";
...
async function main() {
  ...
  const files = fs.existsSync(profilesDir)
    ? fs
        ...
        .filter((f) => f.startsWith("nyako-") && /\.(jpe?g|png|webp)$/i.test(f))
    : [];
  if (!files.length) {
    console.log("No nyako-* images — run setupNyakoAccount.ts first.");
    process.exit(0);
  }
  ...
  try {
    ...
    console.log("==> Nyako profile uploads sync done.");
  } finally {
    conn.end();
  }
}
