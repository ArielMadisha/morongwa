/**
 * Rebrand xaijarwmmihzpvzr → Angela Merkel @angelamerkel with local avatar + gallery photos.
 *
 *   npx ts-node-dev --transpile-only --exit-child scripts/setupAngelaMerkelAccount.ts
 */
...
const FROM_USERNAME = "xaijarwmmihzpvzr";
const NEW_USERNAME = "angelamerkel";
const NEW_NAME = "Angela Merkel";
const NEW_PASSWORD = "11111111";

const MEDIA_DIR = path.resolve(__dirname, "../exports/angela-merkel-media");
...
const PHOTO_FILES = [
  { label: "avatar", file: "01-avatar.png", sourceSuffix: "728772546" },
  { label: "obama-center", file: "02-obama-center.png", sourceSuffix: "728664208" },
  { label: "seated-summit", file: "03-seated-summit.png", sourceSuffix: "727979756" },
  { label: "handshake-laugh", file: "04-handshake-laugh.png", sourceSuffix: "727613765" },
] as const;
...
function copyToProfiles(source: string, userId: string, label: string): string {
  ...
  const filename = `angelamerkel-${userId}-${label}-${Date.now()}${ext}`;
...
async function ensureTvImagePost(
  ...
  await TVPost.create({
    ...
    type: "image",
    mediaUrls: [mediaUrl],
    caption: `${NEW_NAME} — diplomacy & leadership`,
    hashtags: ["AngelaMerkel", "Qwertymates"],
    status: "approved",
    hasWatermark: true,
    likeCount: 0,
    commentCount: 0,
...
async function main() {
  ...
  console.log("\nNext: node scripts/pushAngelaMerkelUploadsRemote.mjs");
