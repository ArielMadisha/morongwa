/**
 * Rebrand techwareuser → Terekeka Sudan @terekekasudan with local avatar + gallery photos.
 *
 *   npx ts-node-dev --transpile-only --exit-child scripts/setupTerekekaSudanAccount.ts
 */
...
const FROM_USERNAME = "techwareuser";
const NEW_USERNAME = "terekekasudan";
const NEW_NAME = "Terekeka Sudan";
const NEW_PASSWORD = "11111111";

const MEDIA_DIR = path.resolve(__dirname, "../exports/terekeka-sudan-media");
...
const PHOTO_FILES = [
  { label: "avatar", file: "01-avatar.png", sourceSuffix: "726001941" },
  { label: "flower-headpieces", file: "02-flower-headpieces.png", sourceSuffix: "727236748" },
  { label: "clay-headpiece", file: "03-clay-headpiece.png", sourceSuffix: "728427440" },
  { label: "smiling-portrait", file: "04-smiling-portrait.png", sourceSuffix: "728512022" },
] as const;
...
function copyToProfiles(source: string, userId: string, label: string): string {
  ...
  const filename = `terekekasudan-${userId}-${label}-${Date.now()}${ext}`;
...
async function ensureTvImagePost(
  ...
  await TVPost.create({
    ...
    type: "image",
    mediaUrls: [mediaUrl],
    caption: `${NEW_NAME} — South Sudan culture & heritage`,
    hashtags: ["TerekekaSudan", "SouthSudan", "Qwertymates"],
    status: "approved",
    hasWatermark: true,
    likeCount: 0,
    commentCount: 0,
...
async function main() {
  ...
  console.log("\nNext: node scripts/pushTerekekaSudanUploadsRemote.mjs");
