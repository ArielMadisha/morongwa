/**
 * Sync QWERTZ_API_URL + QWERTZ_API_KEY from local backend/.env → remote backend/.env, restart API.
 *
 * From backend/:
 *   npm run sync:qwertz-env-remote
 *   npm run sync:qwertz-env-remote -- --dry-run
 */
import fs from "fs";
...
import { buildQwertzEnvUpdates, upsertLocalEnvFile } from "./lib/qwertzDeploy.mjs";
...
async function main() {
  ...
  const updates = buildQwertzEnvUpdates(local, cfg);

  if (!updates.QWERTZ_API_KEY) {
    throw new Error("QWERTZ_API_KEY missing — run deploy:qwertz-remote once to generate");
  }

  upsertLocalEnvFile(localEnvPath, updates);

  console.log("==> Qwertz keys to sync:");
  console.log(`  QWERTZ_API_URL=${updates.QWERTZ_API_URL}`);
  console.log(`  QWERTZ_API_KEY=(set, ${updates.QWERTZ_API_KEY.length} chars)`);
