/**
 * SFTP aaroncollier-* profile images to production.
 *   node scripts/pushAaronCollierUploadsRemote.mjs
 */
import fs from "fs";
...
async function main() {
  ...
  const files = fs.existsSync(profilesDir)
    ? fs
        ...
        .filter((f) => f.startsWith("aaroncollier-") && /\.(jpe?g|png|webp)$/i.test(f))
    : [];
  if (!files.length) {
    console.log("No aaroncollier-* images — run setupAaronCollierAccount.ts first.");
    process.exit(0);
  }
  ...
  try {
    ...
    console.log("==> Aaron Collier profile upload sync done.");
  } finally {
    conn.end();
  }
}
