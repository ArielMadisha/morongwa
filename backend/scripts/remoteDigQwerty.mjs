...
async function main() {
  ...
  const out = await execSsh(
    conn,
    "dig +short qwertymates.com A; echo ---; dig +short www.qwertymates.com A; echo ---; dig +short www.qwertymates.com CNAME; curl -sI --max-time 10 -H 'Host: qwertymates.com' http://127.0.0.1/wall 2>&1 | head -8"
  );
  conn.end();
  console.log(out);
}
