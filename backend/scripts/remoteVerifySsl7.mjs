...
async function main() {
  ...
  const out = await execSsh(
    conn,
    "docker exec nginx-app-1 sh -lc 'ls -la /data/nginx/proxy_host/7.conf; echo ---; echo | openssl s_client -connect 127.0.0.1:443 -servername www.qwertymates.com 2>/dev/null | openssl x509 -noout -subject -dates 2>/dev/null'"
  );
  conn.end();
  console.log(out);
}
