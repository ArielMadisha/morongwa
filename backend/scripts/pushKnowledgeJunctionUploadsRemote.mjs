/**
 * SFTP knowledgejunction-* profile images to production.
 *   node scripts/pushKnowledgeJunctionUploadsRemote.mjs
 */
import fs from "fs";
...
async function main() {
  ...
  const files = fs.existsSync(profilesDir)
    ? fs.readdirSync(profilesDir).filter((f) => f.startsWith("knowledgejunction-") && /\.(jpe?g|png|webp)$/i.test(f))
    : [];
  if (!files.length) {
    console.log("No knowledgejunction-* images — run setupKnowledgeJunctionAccount.ts first.");
    process.exit(0);
  }
  ...
  try {
    ...
    console.log("==> Knowledge Junction profile upload sync done.");
  } finally {
    conn.end();
  }
}
