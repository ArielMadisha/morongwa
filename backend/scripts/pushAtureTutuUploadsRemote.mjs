/**
 * SFTP aturetutu-* profile images to production.
 *   node scripts/pushAtureTutuUploadsRemote.mjs
 */
import fs from "fs";
...
async function main() {
  ...
  const files = fs.existsSync(profilesDir)
    ? fs
        ...
        .filter((f) => f.startsWith("aturetutu-") && /\.(jpe?g|png|webp)$/i.test(f))
    : [];
  if (!files.length) {
    console.log("No aturetutu-* images — run setupAtureTutuAccount.ts first.");
    process.exit(0);
  }
  ...
  try {
    ...
    console.log("==> Ature Tutu profile uploads sync done.");
  } finally {
    conn.end();
  }
}
