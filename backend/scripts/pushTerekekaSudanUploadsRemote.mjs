/**
 * SFTP terekekasudan-* profile images to production.
 *   node scripts/pushTerekekaSudanUploadsRemote.mjs
 */
import fs from "fs";
...
async function main() {
  ...
  const files = fs.existsSync(profilesDir)
    ? fs
        ...
        .filter((f) => f.startsWith("terekekasudan-") && /\.(jpe?g|png|webp)$/i.test(f))
    : [];
  if (!files.length) {
    console.log("No terekekasudan-* images — run setupTerekekaSudanAccount.ts first.");
    process.exit(0);
  }
  ...
  try {
    ...
    console.log("==> Terekeka Sudan profile uploads sync done.");
  } finally {
    conn.end();
  }
}
