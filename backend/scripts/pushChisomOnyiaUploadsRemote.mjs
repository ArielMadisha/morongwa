/**
 * SFTP chisomonyia-* profile images to production.
 *   node scripts/pushChisomOnyiaUploadsRemote.mjs
 */
import fs from "fs";
...
async function main() {
  ...
  const files = fs.existsSync(profilesDir)
    ? fs
        ...
        .filter((f) => f.startsWith("chisomonyia-") && /\.(jpe?g|png|webp)$/i.test(f))
    : [];
  if (!files.length) {
    console.log("No chisomonyia-* images — run setupChisomOnyiaAccount.ts first.");
    process.exit(0);
  }
  ...
  try {
    ...
    console.log("==> Chisom Onyia profile uploads sync done.");
  } finally {
    conn.end();
  }
}
