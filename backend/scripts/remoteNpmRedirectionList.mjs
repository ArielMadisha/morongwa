...
cur = c.cursor()
cur.execute(
  "SELECT id, domain_names, forward_domain_name, forward_scheme, forward_http_code, preserve_path, certificate_id, enabled, ssl_forced, is_deleted FROM redirection_host ORDER BY id"
)
...
async function main() {
  ...
  const out = await execSsh(conn, `docker exec nginx-app-1 sh -c 'echo ${JSON.stringify(b64)} | base64 -d | python3'`);
  conn.end();
  console.log(out);
}
