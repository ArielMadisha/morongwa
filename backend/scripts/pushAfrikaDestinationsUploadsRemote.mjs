/**
 * SFTP afrikadestinations-* profile images to production.
 *   node scripts/pushAfrikaDestinationsUploadsRemote.mjs
 */
import fs from "fs";
...
async function main() {
  ...
  const files = fs.existsSync(profilesDir)
    ? fs
        ...
        .filter((f) => f.startsWith("afrikadestinations-") && /\.(jpe?g|png|webp)$/i.test(f))
    : [];
  if (!files.length) {
    console.log("No afrikadestinations-* images — run setupAfrikaDestinationsAccount.ts first.");
    process.exit(0);
  }
  ...
  try {
    ...
    console.log("==> AfrikaDestinations profile uploads sync done.");
  } finally {
    conn.end();
  }
}
