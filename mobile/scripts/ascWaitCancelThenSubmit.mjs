/**
 * Wait until UNRESOLVED submission finishes canceling, then submit 1.0 with new build.
 */
import fs from "fs";
...
let unresolvedState = null;
for (let i = 0; i < 16; i++) {
  const sub = await asc(token, "GET", `/v1/reviewSubmissions/${UNRESOLVED_SUB}`);
  unresolvedState = sub.data.attributes?.state;
  console.log("unresolved state", unresolvedState);
  if (["COMPLETE", "CANCELLED", "CANCELED"].includes(unresolvedState)) break;
  await sleep(20_000);
}
...
console.log("add to new sub", add.ok, add.status || 200, JSON.stringify(add.body || add.json)?.slice(0, 800));
...
console.log("submit new", submit.ok, submit.status || 200, JSON.stringify(submit.body || submit.json)?.slice(0, 800));
...
fs.writeFileSync(
  path.join(process.cwd(), "exports", "asc-wait-cancel-submit-result.json"),
